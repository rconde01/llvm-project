...
| `procedure`            | object           | On the Name of a SUBROUTINE / FUNCTION (any symbol with `SubprogramDetails`).  Carries `is_function` (boolean), an ordered `dummy_args` array of `{name, type, rank, attrs?}` (alternate-return placeholders show as `{"alternate_return":true}`), and for functions a `result` object of the same shape.  Each dummy's `attrs` is the subset of intent / `optional` / `value` / `pointer` / `allocatable` / `target` flags relevant to call-site reasoning. |
