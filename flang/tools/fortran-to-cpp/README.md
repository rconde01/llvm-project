...
(`-fdebug-dump-analyzed-tree-json`) via the typed loader in
...
4. **Minimize heuristics.** The converter must prefer *facts* that
   ...
   by the JSON parse-tree dumper (see `dump-analyzed-tree-json.h`, which
...
### D8 — Dummy procedures **(resolved: function templates + generic-lambda actuals)**

A procedure passed as an argument has no interface under FORTRAN 77, so the
receiving routine genuinely does not know the callback's arity or argument
types.  Three options were considered:

  * **`std::function<R(args)>`** — one fixed, separately-compilable
    parameter type, but it pins down `R(args)`, forcing the converter to
    *reconstruct* the signature across the whole call graph.  That inference
    is fragile for **forwarded-only** callbacks (passed straight on, never
    called locally → no local clue) and **nested** ones (a procedure whose
    own argument is a procedure), and it cannot be made consistent where a
    concrete actual and a forwarded one meet.
  * **function pointer** — zero overhead, but cannot carry the
    state-capturing lambda the actual needs (threaded COMMON/SAVE state).
  * **function template `<class F>`** *(chosen)* — each dummy procedure is a
    *deduced* parameter `const F<k>&`; the compiler infers the callback type
    from whatever is passed, so there is **no signature inference at all**,
    and forwarded/nested chains just work.

Every procedure *actual* is wrapped in a **state-capturing generic lambda**
`[&](auto&&... a){ actual(state…, std::forward<decltype(a)>(a)...); }`.
This is what makes deduction always succeed: the lambda is a concrete
closure (so even a higher-order routine — itself a template, with no
`decltype`able name — can be passed), it captures state by reference, and
being generic it imposes no fixed signature.

**Build-model consequence.** A template's definition must be visible at
every call site, so a procedure-taking routine's *definition* moves from its
own `.cpp` into the **shared header** (the per-file `.cpp` emits everything
else as before).  Cost: more code instantiated per call-type and slower
compiles for that subsystem; benefit: the cross-program dummy-procedure
inference pass is gone entirely.  See **Dummy procedures** in
`docs/CONSTRUCTS.md` for the full treatment.
